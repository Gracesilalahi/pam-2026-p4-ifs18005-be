...
import org.delcom.services.DollService // Import DollService
import org.koin.ktor.ext.inject

fun Application.configureRouting() {
    val plantService: PlantService by inject()
    val profileService: ProfileService by inject()
    val dollService: DollService by inject() // Inject DollService
    ...
    routing {
        get("/") {
            call.respondText("API telah berjalan. Dibuat oleh Grace Sania Silalahi.")
        }
        ...
        // Route Dolls (Versi Boneka)
        route("/dolls") {
            get {
                dollService.getAllDolls(call)
            }
            post {
                dollService.createDoll(call)
            }
            get("/{id}") {
                dollService.getDollById(call)
            }
            put("/{id}") {
                dollService.updateDoll(call)
            }
            delete("/{id}") {
                dollService.deleteDoll(call)
            }
            get("/{id}/image") {
                dollService.getDollImage(call)
            }
        }
